...
bool HandleBulletPenetration(FireBulletData &data, CSWeaponInfo *wpn_data);
...
void ScaleDamage(IClientEntity *enemy, float weapon_armor_ratio, float &current_damage, int hitgroup)
{
...
bool SimulateFireBullet(CBaseCombatWeapon *weapon, FireBulletData &data, IClientEntity *local)
{
	...
	while ((data.penetrate_count > 0) && (data.current_damage >= 1.0f))
	{
			...
			/*&& (DoesRayTouchHitbox(end, &data))*/)
		{
			...
			ScaleDamage(data.enter_trace.m_pEnt, wpn_data->m_flArmorRatio, data.current_damage, data.enter_trace.hitgroup);

			return true;
		}

		if (!HandleBulletPenetration(data, wpn_data))
			break;
	}
...
bool CanHit(const Vector &point, float *damage_given)
{
	...
	if (SimulateFireBullet((CBaseCombatWeapon*)Interfaces::EntList->GetClientEntityFromHandle((HANDLE)local->GetActiveWeaponHandle()), data, local))
	{
