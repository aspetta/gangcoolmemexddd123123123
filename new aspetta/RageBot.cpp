...
// Functionality
void CRageBot::DoAimbot(bool &bSendPacket, CUserCmd *pCmd) // Ragebot, Aimbot
{
...
// Las równikowy:
//wysoka temperatura, intensywne deszcze zawsze o tej samej porze, parowanie wody, inna flora i fauna, brak pór roku
//Las liœciasty :
//zrzucaj¹ liœcie na zimê, inna flora i fauna, umiarkowane temperatury
...
namespace AntiAims // CanOpenFire checks for fake anti aims?
{
	...
	void FakeEdge(bool &bSendPacket, CUserCmd *pCmd)
	{
		IClientEntity* pLocal = hackManager.pLocal();
		...
		for (int y = 0; y < 360; y++)
		{
			...
			int offsetpitch = Menu::Window.RageBotTab.AntiAimPitchoffset.GetValue();
	...
	void FakeSideways(bool &bSendPacket, CUserCmd *pCmd)
	{
		static int ChokedPackets = -1;
		ChokedPackets++;
		if (ChokedPackets < 1)
	...
	void fakeback(CUserCmd *pCmd, bool bSendPacket)
	{
		...
		else
		{
			bSendPacket = false;
			pCmd->viewangles.y += 90;
			faked = true; //real angle backwards suck my cock pCoders =)
		}
	}
	...
	void FastSpintFake(bool& bSendPacket, CUserCmd *pCmd)
	{
	...
	void lby(CUserCmd *pCmd, bool &bSendPacket)
	{
// a kurwa nie ma 
...
void CRageBot::DoAntiAim(CUserCmd *pCmd, bool &bSendPacket) // pCmd->viewangles.y = 0xFFFFF INT_MAX or idk
{
	...
	case 1:
		// Fake Inverse
		AntiAims::FakeEdge(bSendPacket, pCmd);
		break;
	case 2:
		// Fake Sideways
		if (bSendPacket)
	...
	case 16:
		...
		break;
	case 17:
		AntiAims::FastSpintFake(bSendPacket, pCmd);
		break;
	...
	case 0:
		//off
		break;
	// removed bo anti pasta wy biedaki XD
	...
	// Angle offset pitch
	pCmd->viewangles.x += Menu::Window.RageBotTab.AntiAimPitchoffset.GetValue();
}
