...
void __stdcall Hooked__PlaySoundCSGO(const char* fileName)
{
	...
	{
		//Accept the game
		uintptr_t IsReadyEx = Utilities::Memory::FindPattern("client.dll", (PBYTE)"55 8B EC 51 56 8B 35 ? ? ? ? 80 7E 58 00", "xxxxxxxxx????xxxxxx");
		//GameUtils::IsReady = (IsReadyFn)(Offsets::Functions::dwIsReady);
		GameUtils::IsReady = reinterpret_cast<IsReadyFn>(IsReadyEx);
		GameUtils::IsReady();
		...
		fi.uCount = 0;// to crashuje ten autoaccept
		fi.dwTimeout = 0;
		FlashWindowEx(&fi);
	}
}
